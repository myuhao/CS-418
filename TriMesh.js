    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        for (var i = 0; i<lines.length; i++){
            var token = lines[i].replace('\r', '').split(' ');
            //console.log(token)
            ...
            //Push to the vBuffer
            if (token[0] == "v") {
                this.vBuffer.push(parseFloat(token[1]));
                this.vBuffer.push(parseFloat(token[2]));
                this.vBuffer.push(parseFloat(token[3]));
                this.numVertices++;
            }
            //Push to the fBuffer
            if (token[0] == "f") {
                this.fBuffer.push(parseInt(token[2]) - 1);
                this.fBuffer.push(parseInt(token[3]) - 1);
                this.fBuffer.push(parseInt(token[4]) - 1);
                this.numFaces++;
            }
